...
- Rubyのクラスは定数に紐付けられた普通にオブジェクトである
...
[ハッシュ Hash]({% post_url 2017-07-22-ハッシュ %}) の辺りも重要
...
- 内側から参照、変更ができる
- やる気になれば外部からもさわれる
- [変数、定数]({% post_url 2017-07-22-変数、定数 %}#インスタンス変数) の辺りにもちょっと書いてある
...
- 継承したメソッドでインスタンス変数を初期化すると、なんか継承された側に紐付いたようなインスタンス変数が出来るようなそんなイメージが無いだろうか、でも気にしない。そのインスタンス変数の持ち主はそのメソッドを実行した自分（レシーバ）なのだから
...
- オブジェクト外から確認する事ができます
- インスタンスメソッドはクラスに、インスタンス変数はオブジェクトに存在するのでしたね、覚えてますか？

クラス#instance_methods
...
# 引数にfalseを指定すると、スーパークラスに実装してあるメソッドは表示しない、自分に実装されているものだけを表示する
```
...
- Moko#method_missing は何の意味もない。動作しなかった
...
- rubyインタプリタは継承チェーンを駆け上り、BasicObjectでも見つからないので moko#method_missing(:hage, *['zura']) を実行する
...
生成したモジュールは #include メソッドでクラスに取り込むことができる
...
インスタンス変数へアクセスする専用のメソッドみたいな [変数、定数]({% post_url 2017-07-22-変数、定数 %}#インスタンス変数)
