...
- コードを束縛を好きなように組み合わせるもう一つの方法
  - BasicObject#instance_eval に 渡したブロックはselfがレシーバに変更されてから実行される
...
- クラス名が分からない、評価時にクラスが確定する・・という状況でクラスをオープンしたい
  ...
  - ちゃんと説明するには特異クラスが必要になります、今は
    - instance_eval は selfを変更してくれるんで「インスタンスをオープンする、カレントクラスは気にしねえよ」って時
    - class_eval は selfとカレントクラスを変更してくれるんで「インスタンスをオープンする、カレントクラスも変える」って時
    - に使う、ような感じでイメージしておく
- クラスインスタンス変数
  ...
  - 特異クラス
    ...
    - さて、特異クラスはもちろんクラスだ。しかして特別なクラスだ
      ...
      - ```ruby
        ...
        ```
  - メソッド探索 再び
    - というか、[ここ]({% post_url 2017-07-22-Gold試験時に意識した要素-Ruby技術者認定試験の書籍写経メモ %}) にまとめてしまったのでここを見ればほとんど書いてある
    - 
